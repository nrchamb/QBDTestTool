...
def setup_setup_subtab(app):
    ...
    # Load All button - at top for primary action
    load_all_frame = ttk.Frame(content)
    load_all_frame.pack(fill='x', pady=(0, SPACING_SM))
    ...
    # Load Previous Session button - only show if session exists
    if SessionManager.session_exists():
        session_info = SessionManager.get_session_info()
        item_count = session_info.get('total_items', 0) if session_info else 0
        load_session_frame = ttk.Frame(content)
        load_session_frame.pack(fill='x', pady=(SPACING_SM, 0))
        app.setup_load_session_btn = ttk.Button(
            load_session_frame,
            text=f"Load Previous Session ({item_count} items)",
            command=app._load_session_now
        )
        app.setup_load_session_btn.pack()
