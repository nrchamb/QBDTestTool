...
def setup_transaction_subtab(app):
    ...
    # Save defaults function (defined before button frame)
    def save_transaction_defaults():
        """Save current values as defaults."""
        try:
            amount_min = int(app.txn_amount_min.get())
            amount_max = int(app.txn_amount_max.get())
            line_items_min = int(app.txn_lines_min.get())
            line_items_max = int(app.txn_lines_max.get())
            AppConfig.save_transaction_defaults(amount_min, amount_max, line_items_min, line_items_max)
            app._log_create("Transaction defaults saved")
        except ValueError:
            app._log_create("Error: Invalid numeric values for defaults")

    # Button row at top for quick access
    button_frame = ttk.Frame(content)
    button_frame.pack(pady=(0, SPACING_LG))
    ...
    app.create_transaction_btn.pack(side='left')

    # Spacer for separation
    ttk.Frame(button_frame, width=SPACING_XL * 3).pack(side='left', padx=SPACING_XL)

    ttk.Button(
        button_frame,
        text="Save as Defaults",
        command=save_transaction_defaults
    ).pack(side='left')
    ...
    # Load saved transaction defaults
    txn_defaults = AppConfig.get_transaction_defaults()
    ...
    app.txn_amount_min.insert(0, str(txn_defaults.get('amount_min', 25)))
    ...
    app.txn_amount_max.insert(0, str(txn_defaults.get('amount_max', 500)))
