...
def _format_delete_debug(txn_type: str, txn_id: str, result: dict) -> str:
    """Format deletion operation debug info for logging."""
    if result.get('success'):
        return f"OK - txn_type={txn_type}, txn_id={txn_id}"
    else:
    ...
    try:
        ...
        for inv in archived_invoices:
            try:
                ...
                result = qb.execute_operation('delete_txn', {'txn_type': 'Invoice', 'txn_id': inv.txn_id})

                # DEBUG: Log the operation (only if DEBUG is enabled)
                if should_log(LOG_DEBUG, AppConfig.get_log_level()):
                    debug_str = _format_delete_debug('Invoice', inv.txn_id, result)
                    app.root.after(0, lambda ref=inv.ref_number, d=debug_str:
                                  app._log_create(f"  [DEBUG Invoice {ref}] {d}", LOG_DEBUG))
        ...
        for sr in archived_receipts:
            try:
                ...
                # Send to QuickBooks via QBFC
                result = qb.execute_operation('delete_txn', {'txn_type': 'SalesReceipt', 'txn_id': sr.txn_id})

                # DEBUG: Log the operation (only if DEBUG is enabled)
                if should_log(LOG_DEBUG, AppConfig.get_log_level()):
                    debug_str = _format_delete_debug('SalesReceipt', sr.txn_id, result)
                    app.root.after(0, lambda ref=sr.ref_number, d=debug_str:
                                  app._log_create(f"  [DEBUG Receipt {ref}] {d}", LOG_DEBUG))
        ...
        for sc in archived_charges:
            try:
                ...
                # Send to QuickBooks via QBFC
                result = qb.execute_operation('delete_txn', {'txn_type': 'Charge', 'txn_id': sc.txn_id})

                # DEBUG: Log the operation (only if DEBUG is enabled)
                if should_log(LOG_DEBUG, AppConfig.get_log_level()):
                    debug_str = _format_delete_debug('Charge', sc.txn_id, result)
                    app.root.after(0, lambda ref=sc.ref_number, d=debug_str:
                                  app._log_create(f"  [DEBUG Charge {ref}] {d}", LOG_DEBUG))
