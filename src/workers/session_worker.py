...
from app_logging import LOG_NORMAL, LOG_DEBUG, LOG_VERBOSE
...
def save_session_worker(app, silent=False):
    ...
    try:
        ...
        # DEBUG: Log session contents being saved
        if should_log(LOG_DEBUG, AppConfig.get_log_level()):
            cust_count = len(state.customers)
            inv_count = len(state.invoices)
            sr_count = len(state.sales_receipts)
            sc_count = len(state.statement_charges)
            app.root.after(0, lambda cu=cust_count, i=inv_count, s=sr_count, c=sc_count:
                          app._log_create(f"  [DEBUG Session] Saving: {cu} customers, {i} invoices, {s} sales receipts, {c} charges", LOG_DEBUG))
...
def load_session_worker(app, verify_changes=False):
    ...
    try:
        ...
        # DEBUG: Log session contents being loaded
        if should_log(LOG_DEBUG, AppConfig.get_log_level()):
            inv_count = len(session_data.get('invoices', []))
            sr_count = len(session_data.get('sales_receipts', []))
            sc_count = len(session_data.get('statement_charges', []))
            cust_count = len(session_data.get('customers', []))
            app.root.after(0, lambda i=inv_count, s=sr_count, c=sc_count, cu=cust_count:
                          app._log_create(f"  [DEBUG Session] Loading: {cu} customers, {i} invoices, {s} sales receipts, {c} charges", LOG_DEBUG))
