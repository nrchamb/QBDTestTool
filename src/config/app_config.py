...
DEFAULT_CONFIG = {
    ...
    "ui_state": {
        ...
        "create_log_sash_pos": None,  # Sash ratio (0.0-1.0) for Create tab log (None = use default 0.8)
        "monitor_log_sash_pos": None  # Sash ratio (0.0-1.0) for Monitor tab log (None = use default 0.8)
    },
...
class AppConfig:
    ...
    @staticmethod
    def get_transaction_defaults() -> Dict[str, int]:
        """
        Get transaction default values.

        Returns:
            Dict with amount_min, amount_max, line_items_min, line_items_max
        """
        config = AppConfig.load_config()
        return config.get('transaction_defaults', DEFAULT_CONFIG['transaction_defaults'])

    @staticmethod
    def save_transaction_defaults(amount_min: int, amount_max: int,
                                   line_items_min: int, line_items_max: int) -> bool:
        """
        Save transaction default values.

        Args:
            amount_min: Minimum transaction amount
            amount_max: Maximum transaction amount
            line_items_min: Minimum line items
            line_items_max: Maximum line items

        Returns:
            True if successful
        """
        config = AppConfig.load_config()
        config['transaction_defaults'] = {
            'amount_min': amount_min,
            'amount_max': amount_max,
            'line_items_min': line_items_min,
            'line_items_max': line_items_max
        }
        return AppConfig.save_config(config)

    @staticmethod
    def get_debug_tools_enabled() -> bool:
        """
        Get debug tools enabled setting.

        Returns:
            True if debug tools should be shown
        """
        config = AppConfig.load_config()
        return config.get('debug', {}).get('tools_enabled', False)

    @staticmethod
    def save_debug_tools_enabled(enabled: bool) -> bool:
        """
        Save debug tools enabled setting.

        Args:
            enabled: Whether to show debug tools
