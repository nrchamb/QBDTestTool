...
Now uses operation-based protocol instead of XML strings.
"""
...
class QBIPCClient:
    ...
    Drop-in replacement for QBConnection class. Now uses QBFC operations instead of QBXML.
    """

    @staticmethod
    def execute_operation(operation: str, params: Dict[str, Any], company_file: Optional[str] = None) -> Dict[str, Any]:
        """
        Execute a QB operation via the connection manager process.

        Args:
            operation: Operation name (e.g., 'add_customer', 'query_invoice')
            params: Operation parameters as dict
            company_file: Optional path to company file

        Returns:
            Dict with operation result (same structure as QBXMLParser output)
